...
app.get('/api/expenses', (req, res) => {
  ...
  if (isNaN(budgetId) || !isValidDate(month)) {
    return res.status(400).json({ error: 'Invalid parameters. Ensure budgetId is a valid integer and month is a valid Date.' });
  }
...
app.get('/expenses', (req, res) => {
  try {
    ...
    if (isNaN(budgetId) || !/^\d{4}-\d{2}$/.test(month)) {
      return res.status(400).json({ error: 'Invalid parameters. Ensure budgetId is a valid integer and month is in the format YYYY-MM.' });
    }
